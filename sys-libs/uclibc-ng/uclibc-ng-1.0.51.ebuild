# Copyright 2025 Avery Terrel <averymt@proton.me>
# Distributed under the terms of the BSD 0-clause license
...
SRC_URI="https://downloads.uclibc-ng.org/releases/${PV}/${P}.tar.gz"
LICENSE="LGPL-2"
SLOT="0"
...
RESTRICT="strip"
# TODO: test whether iconv is still broken
# From the old official (1.0.35) ebuild:
...
#RDEPEND="iconv? ( dev-libs/libiconv )"
...
make_config() {
	...
	for count in 1 2 ; do
		...
		for def in ${defs_y[@]} ; do
			sed -i "s|.*\<${def}\>.*set|${def}=y|g" .config
		done
		for def in ${defs_n[@]} ; do
			sed -i "s|${def}=y|# ${def} is not set|g" .config
		done
		make_oldconfig
	done
...
src_prepare() {
	...
	# where MAJOR_VERSION != 0 indicates the ABI version.
